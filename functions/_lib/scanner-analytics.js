const SCANNER_PATH_BUCKETS = [
  ...
  ["vite-fs-probe", /^\/@fs(?:\/|$)/i],
  ["credential-probe", /^\/(?:.*\/)?(?:\.aws\/(?:config|credentials)|id_rsa|id_dsa|id_ecdsa|id_ed25519|authorized_keys)$/i],
  ["hidden-config", /^\/(?:.*\/)?(?:\.env(?:[._-][a-z0-9-]+)?|\.git|\.svn|\.hg|\.DS_Store)(?:\/|$)/i],
  ["app-config", /^\/(?:__env\.js|env\.js|config\.js|api\/config\.json|config\/[a-z0-9_.-]+\.(?:exs|ini|json|toml|ya?ml)|\.openai\/config\.json|\.gitlab-ci\.ya?ml)$/i],
  ["source-map", /^\/.*\.map$/i],
